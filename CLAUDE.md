...
`@acme-skunkworks/commitlint-config` — the estate's shared [commitlint](https://commitlint.js.org)
...
- **`pre-push`** — blocks direct pushes to `main`; humans should use `/send-it` to open a PR. Bot
  ...
  CI's `reusable-validate-commits` gate is still the authority. This repo dogfoods via
  `commitlint.config.mjs` extending `./dist/index.js` (consumers use the published package name).
...
workflows in `acme-skunkworks/shared-workflows` (floating on `@v1`).
...
`@acme-skunkworks/changelog-core` (`pnpm validate:changelog`, `pnpm exec changelog-core
...
4. Configure Trusted Publisher: `https://www.npmjs.com/package/@acme-skunkworks/commitlint-config/access`
